...
if ($mode=='1')  // neue Anmeldungen
	{
		$abfrage="g.status!='Bearbeitet' and g.status!='Storno'";
...
$sql_course_select = "select g.id,
			...
			g.firstname			AS 'Vorname',
			g.lastname			AS 'Nachname',	
			g.birthdate			AS 'Geburtstag'			
		from 
			web_courseclients g
		where
 			g.id > 0 
...
if ($field!="") 
{
	...
	for ($i=0;$i<sizeof($ff);$i++)
	{
		...
		{
			$sql_course_select.="(g.id LIKE '".$ff1[$ii]."%' 
				   or g.firstname LIKE '".$ff1[$ii]."%'
				   or g.lastname LIKE '".$ff1[$ii]."%'
				   or g.birthdate LIKE '".$ff1[$ii]."%'";
			if ($ii==(sizeof($ff1)-1)) 
			{
				$sql_course_select.=")";
			} else
...
<table border=0 cellspacing=0 cellpadding=0>
	...
	<tr><td width=200 align=center valign=top>
		<SPAN class="headline">Kurs - Online-Anmeldungen-&Uuml;bersicht <br><br> <?print($text)?></SPAN><br>
	</td></tr>
	<tr><td height=5></td></tr>
...
<? if ($pagesize==100000)
{ ?>
    	<a href="webregister_course_list.php?mode=<?echo $mode?>&pagesize=50&field=<?echo $field?>">geteilt anzeigen</a>
<?} else { ?>
	    <a href="webregister_course_list.php?mode=<?echo $mode?>&pagesize=100000&field=<?echo $field?>">alle anzeigen</a>
<?}
?>
	</td></tr>
</table>
...
$links[0]->url = "../admin/webregister_course_form.php?id=";
$links[0]->column = "ID";
$links[0]->param = "";
