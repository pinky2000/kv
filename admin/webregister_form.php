...
if (isset($id) && $id!="" && $no_error && !isset($save_x) && !($update))
{
           $sql_client_select="select   a.id,
										...
										a.rechnung_id 
								from web_clients c,web_camp a where a.web_clients_id=c.id and a.id=".$id;
           $rs_client_select=getrs($sql_client_select,$print_debug,$print_error);
           LIST($web[id],$web[web_clients_id],$web[courses_id],$web[price],
				$web[rab][early],$web[rab][last],$web[rab][stamm],$web[rab][gesch],$web[rab][kombi1],
				$web[rab][kombi2],$web[rab][verl],$web[rab][halb],$web[rab][firmen],$web[rab][sonder],
				$web[opt][verpflegung],$web[opt][nachmittag],$web[opt][nachmittag_auswahl],$web[opt][lernmodul],$web[opt][lernmodul_auswahl],$web[opt][modul3],$web[opt][modul3_auswahl],$web[opt][modul4],$web[opt][modul4_auswahl],
				$web[opt][trans_flug_hin],$web[opt][trans_flug_ret],$web[opt][trans_flug_hin_minor],$web[opt][trans_flug_ret_minor],$web[opt][trans_bahn_hin],$web[opt][trans_bahn_ret],
				$web[opt][zertifikat],$web[opt][zertifikat_auswahl],
...
<?	
	...
	<TR>
		<TD width=150 ALIGN=RIGHT   class=form_header>
		<TABLE>
			<TR><TD WIDTH=100% HEIGHT=100%>gebuchter Kurs</TD></TR>
		</TABLE>
		</TD>
		<td WIDTH=1 class=news_border><IMG SRC="../images/black.gif" HEIGHT=1 WIDTH=1></TD>
		<TD width=250 class=form_row>
		<TABLE>
			<TR><TD WIDTH=100% HEIGHT=100%>
			<SELECT NAME=web[courses_id] onchange="javascript:loadNew();">
   <?
   ...
   while(LIST($camp_id,$prod,$time,$year,$inst,$orig_price)=$rs_camps_select -> fetch_row())
   {
	   ...
	   else
	   {
			...
			<input type="hidden" value="<?print($print_orig_price)?>" name=kurs_price>
			<input type="button" value="->Camp" onclick="javascript:window.open('camp_form.php?id=<?print($web[courses_id])?>')" target=_blank>
			<input type="button" value="->Campblatt" onclick="javascript:window.open('kursblatt_form.php?id=<?print($web[courses_id])?>')" target=_blank>
			</TD></TR>
		</TABLE>
		</TD>
	</TR>
	...
	<TR>
		...
		<TABLE>
			<TR><TD WIDTH=100% HEIGHT=100%>
			<INPUT TYPE=TEXT MAXLENGTH=150 SIZE=30 NAME=web[price] VALUE="<?echo $web[price]?>">
			</TD></TR>
		</TABLE>
		</TD>
	</TR>
...
$rs_courses_optionen_select=getrs("select 
	...
	a.rab_sonder
	from courses a
	where id=".$web[courses_id],$print_debug,$print_error);
	...
	<TR>
		...
		<TABLE>
			...
			<TD WIDTH=50% HEIGHT=100%>
				<nobr>Fr&uuml;hbucher (<?print($rab_early)?> &euro;)<br><input type=text name=web[rab][early] size=8 value="<?print($web[rab][early])?>" onchange="calc();"><br>
				<nobr>Last-Minute (<?print($rab_last)?> &euro;)<br><input type=text name=web[rab][last] size=8 value="<?print($web[rab][last])?>" onchange="calc();"><br>
				<nobr>Stammkunde (<?print($rab_stamm)?> &euro;)<br><input type=text name=web[rab][stamm] size=8 value="<?print($web[rab][stamm])?>" onchange="calc();"><br>
				<nobr>Geschwister (<?print($rab_geschw)?> &euro;)<br><input type=text name=web[rab][gesch] size=8 value="<?print($web[rab][gesch])?>" onchange="calc();"><br>
				<nobr>Kombi 1 (<?print($rab_kombi1)?> &euro;)<br><input type=text name=web[rab][kombi1] size=8 value="<?print($web[rab][kombi1])?>" onchange="calc();"><br>
			</TD>
			<TD WIDTH=50% HEIGHT=100%>
				<nobr>Kombi 2 (<?print($rab_kombi2)?> &euro;)<br><input type=text name=web[rab][kombi2] size=8 value="<?print($web[rab][kombi2])?>" onchange="calc();"><br>
				<nobr>Verl&auml;ngerungswoche (<?print($rab_verl)?> &euro;)<br><input type=text name=web[rab][verl] size=8 value="<?print($web[rab][verl])?>" onchange="calc();"><br>
				<nobr>Nur Halbtags (<?print($rab_halb)?> &euro;)<br><input type=text name=web[rab][halb] size=8 value="<?print($web[rab][halb])?>" onchange="calc();"><br>
				<nobr>Firmenbonus (<?print($rab_firmen)?> &euro;)<br><input type=text name=web[rab][firmen] size=8 value="<?print($web[rab][firmen])?>" onchange="calc();"><br>
				<nobr>Sonderrabatt (<?print($rab_sonder)?> &euro;)<br><input type=text name=web[rab][sonder] size=8 value="<?print($web[rab][sonder])?>" onchange="calc();"><br>
			</TD>
			</TR>
		</TABLE>
		</TD>
	...
	<TR>
		...
		<TABLE>
			...
			<TD WIDTH=100% HEIGHT=100%>
				...
				<input type='text' size=8 name=web[opt][trans_flug_hin] value='<? print($web[opt][trans_flug_hin]);?>' onchange="calc();" ><br>
				<nobr>Flughafen R&uuml;ckfahrt (<?print($opt_flg_ret)?> &euro;)<br>
				<input type='text' size=8  name=web[opt][trans_flug_ret] value='<?print($web[opt][trans_flug_ret]);?>' onchange="calc();" ><br>
				<nobr>Flughafen Hinfahrt Minderj&auml;hrig (<?print($opt_flg_minor_hin)?> &euro;)<br>
				<input type='text' size=8 name=web[opt][trans_flug_hin_minor] value='<?print($web[opt][trans_flug_hin_minor]);?>' onchange="calc();" ><br>
				<nobr>Flughafen R&uuml;ckfahrt Minderj&auml;hrig (<?print($opt_flg_minor_ret)?> &euro;)<br>
				<input type='text' size=8  name=web[opt][trans_flug_ret_minor] value='<?print($web[opt][trans_flug_ret_minor]);?>' onchange="calc();" ><br>
				<nobr>Bahnhof Hinfahrt (<?print($opt_bahn_hin)?> &euro;)<br>
				<input type='text' size=8  name=web[opt][trans_bahn_hin] value='<?print($web[opt][trans_bahn_hin]);?>' onchange="calc();" ><br>
				<nobr>Bahnhof R&uuml;ckfahrt (<?print($opt_bahn_ret)?> &euro;)<br>
				<input type='text' size=8  name=web[opt][trans_bahn_ret] value='<?print($web[opt][trans_bahn_ret]);?>' onchange="calc();" ><br>
			</TD>
			</TR>
		</TABLE>
		</TD>
	...
	<TR>
		...
		<TABLE>
			...
			<TD WIDTH=40% HEIGHT=100%>
				<nobr>Modul 1  (<?print($opt_nachmittag)?> &euro;/Modul) / ausgew&auml;hlte Werte: <input type='text' size=8 name=web[opt][nachmittag] value='<?print($web[opt][nachmittag]);?>' onchange="calc();"><br>
<? //print($opt_nachmittag_auswahl." -- ".$web[opt][nachmittag_auswahl]."<br>")?>
				<select multiple name=opt_nachmittag_select[]>
<?
$m1_trim_selected=trim($web[opt][nachmittag_auswahl],";");
...
<br>
				<nobr>Modul 2  (<?print($opt_lernmodul)?> &euro;/Modul) / ausgew&auml;hlte Werte: <input type='text' size=8 name=web[opt][lernmodul] value='<?print($web[opt][lernmodul]);?>' onchange="calc();"><br>
				<select multiple name=opt_lernmodul_select[]>
<?
$m1_trim_selected=trim($web[opt][lernmodul_auswahl],";");
$m1_split_selected=split(";",$m1_trim_selected);
...
<br>
				<nobr>Modul 3  (<?print($opt_modul3)?> &euro;/Modul) / ausgew&auml;hlte Werte: <input type='text' size=8 name=web[opt][modul3] value='<?print($web[opt][modul3]);?>' onchange="calc();"><br>
				<select multiple name=opt_modul3_select[]>
<?
$m1_trim_selected=trim($web[opt][modul3_auswahl],";");
$m1_split_selected=split(";",$m1_trim_selected);
...
<br>
				<nobr>Modul 4  (<?print($opt_modul4)?> &euro;/Modul) / ausgew&auml;hlte Werte: <input type='text' size=8 name=web[opt][modul4] value='<?print($web[opt][modul4]);?>' onchange="calc();"><br>
				<select multiple name=opt_modul4_select[]>
<?
$m1_trim_selected=trim($web[opt][modul4_auswahl],";");
$m1_split_selected=split(";",$m1_trim_selected);
...
<option value=''> </option>
					...
					<br><br>
				<nobr>Verpflegung  (<?print($opt_verpflegung)?> &euro;)<br><input type='text' size=8 name=web[opt][verpflegung] value='<?print($web[opt][verpflegung]);?>' onchange="calc();" ><br>
				<nobr>Zertifikat  (<?print($opt_zertifikat)?> &euro;) <input type='text' size=8 name=web[opt][zertifikat] value='<?print($web[opt][zertifikat]);?>' onchange="calc();" ><br>
				<SELECT NAME=web[opt][zertifikat_auswahl]>
					<OPTION VALUE="1" <?if ($web[opt][zertifikat_auswahl]=="1") print("SELECTED");?>>A1-Anf&auml;nger I</OPTION>
					<OPTION VALUE="2" <?if ($web[opt][zertifikat_auswahl]=="2") print("SELECTED");?>>A2-Anf&auml;nger II</OPTION>
					<OPTION VALUE="3" <?if ($web[opt][zertifikat_auswahl]=="3") print("SELECTED");?>>B1-Fortgeschritten I</OPTION>
					<OPTION VALUE="4" <?if ($web[opt][zertifikat_auswahl]=="4") print("SELECTED");?>>B2-Fortgeschritten II</OPTION>
					<OPTION VALUE="5" <?if ($web[opt][zertifikat_auswahl]=="5") print("SELECTED");?>>C1-Native</OPTION>
					<option value='0' <?if (($web[opt][zertifikat_auswahl]=="0") || ($web[opt][zertifikat_auswahl]=="")) print("SELECTED");?>> </option>
				</SELECT>
