...
<html>
	<head>
        <title>Kursverwaltung <?print($save_x);?></title>
...
if (isset($delete_x))
{
	if ($confirm=="true" && $id>0)
	{
		...
		echo "<BODY><div><center><b>Kurs wurde gel�scht!</b><br></center></div></BODY>";
		die;
	}
}
...
{
  document.kurse.confirm.value = confirm("Wollen Sie diesen Eintrag wirklich l�schen?");
}
...
<?
    if ($no_error)
    {
        ...
        for ($zeile=0;$zeile<=$anzahl_termine;$zeile++)
        {
			...
			// Verf�gbare Felder: kurstermine[x] [del] [id] [datum] [zeit] [dauer]
...
//                    $rs_coursetimes_employees_delete=getrs($sql_coursetimes_employees_delete,$print_debug,$print_error);
				...
				// Verf�gbare Felder: kurstermine[x] [mitarbeiter] [y] [mitarbeiter_id]  [hourcost]  [table_id]
				...
				// Termine und Mitarbeiterzuordnung l�schen
...
if ($kopie)
{
	...
	$sql_kopie_kurs = "INSERT INTO courses
			  ( products_id,year,timeperiods_id,institutions_id,locations_id,info,price,status,standard_time,standard_durance,standard_employee,standard_employee1,standard_employee2,min_clients,remarks,durance_desc,weekday,intern_remarks,jbetrag,semrabatt,jahrrabatt,type, standard_employee3,online_visible )
			  ( SELECT
				  products_id,year,timeperiods_id,institutions_id,locations_id,info,price,status,standard_time,standard_durance,standard_employee,standard_employee1,standard_employee2,min_clients,remarks,durance_desc,weekday,intern_remarks,jbetrag,semrabatt,jahrrabatt,type, standard_employee3,online_visible
			    FROM courses
			    WHERE id = '$id'
	...
	$sql_kurszeiten_update = "UPDATE courses SET info='".$info."-KOPIE', intern_remarks='Kopie von Kurs ".$id.", erstellt am ".$change_date." von ".$change_user."',change_date='$change_date',change_user='$change_user' where id='$kopie_id'";
    ...
    while ($rs_s_kurszeiten>0 && List($kid) = $rs_s_kurszeiten -> fetch_row())
	{
		$sql_kopie_kurszeiten = "INSERT INTO coursetimes
			  ( date,time,durance,courses_id )
			  ( SELECT
				  date, time, durance, $kopie_id
			    FROM coursetimes
			    WHERE id = '$kid'
			  )";
...
*/		
	...
	echo "<div align=center><font size=3 color=red align=center>Kopie war erfolgreich! - ID: $kopie_id</font></div>";die;
// Kopie wird geladen ...
$id=$kopie_id;
}
// ENDE kopie-----------------------------------------------------------------------------------
...
if (($id > 0) && ($update==0))
{
      ...
      List($products_id,$jahr,$time_id,$ort_id,$auftrag_id,$info,$preis,$uhrzeit_0,$dauer_0,$standard_mitarbeiter[0],$standard_mitarbeiter[1],$standard_mitarbeiter[2],$standard_mitarbeiter[3],$st_hc,$minteil,$remarks,$code,$durance_desc,$tag,$intern_remarks,$jbetrag,$semrabatt,$jahrrabatt,$kursinfo,$online_visible) = $rs_kurs_select -> fetch_row();
...
if ($print_debug==1) { print("Termin: ".$anzahl_termine."-".$kurstermine[$anzahl_termine][id]);var_dump($kurstermine[$anzahl_termine][mitarbeiter]);print("<br>"); }	
				...
				// H�ckchen bez. L�schen von Terminen zur�cksetzen
				$kurstermine[$anzahl_termine][del]="0";

                $anzahl_termine++; // N�chster Kurstermin
        }
	$anzahl_termine--;
...
// Mitarbeiter und die Stundens�tze der Kurstermine mit den Standard-Mitarbeitern und Standard-Stundensatz bei �nderungen dieser �ebrschreiben //
...
//Uhrzeit bei allen Kursterminen �berschreiben, wenn Standardwert ge�ndert wird //
...
//Dauer bei allen Kursterminen �berschreiben, wenn Standardwert ge�ndert wird //
...
function autocount()
{
	...
	{	
	var nexttime = firsttime + (7 * 24 * 60 * 60 * 1000);
	        ...
	        <tr height=30>
                ...
                <td>
		            <select name="time_id" class="input_text">
                        <option value=-1></option>
<?
	$sql_time_select="select id,name from timeperiods where status='Aktiv' and camp='0' order by name asc";
...
<? } ?>
    ...
    <tr height=30>
	    ...
	    <a href="javascript:newDate()" class="text_link">-> weiterer Termin <-</a> (max. 20 Termine m�glich!)
<!--         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="javascript:newEmployee()" class="text_link">-> weiterer Mitarbeiter <-</a>	  -->   </td> 
	</TR>
    <tr>
...
<?
	for ($zeile=0;$zeile<=$anzahl_termine;$zeile++)
    { 
		// bei neuen n, m�ssen die Felder mit Werten gef�llt werden
...
} // Ende for f�r Mitarbeiter?>
     </tr>
<? 
} // Ende for f�r Termine?>
	</table>
	</td>
	</tr>
	<tr height=5>
