...
/* Dateiname: timeperiods_list.php
*  Zweck: Übersichtsliste über die Zeitperioden mit Filter- und Sortierfunktion
*/
...
$sql_timeperiods_list  = "SELECT   id,
				   					id 				AS 'ID',
				                   name    			    AS 'Zeitperiode',
				                   status   			AS 'Status'
					     FROM
...
if ($field!="") 
{
	...
	for ($i=0;$i<sizeof($ff);$i++)
	{
		...
		{
			$sql_timeperiods_list.="(name LIKE '".$ff1[$ii]."%' 
				   or id LIKE '".$ff1[$ii]."%'
				   or status LIKE '".$ff1[$ii]."%'";
			if ($ii==(sizeof($ff1)-1)) 
			{
				$sql_timeperiods_list.=")";
			} else
			{
				$sql_timeperiods_list.=") and ";
			}
		...
		{
			$sql_timeperiods_list.=" or ";
		}
	}
}

$rs_timeperiods_list = getrs($sql_timeperiods_list,$print_debug,$print_error);
...
<? if ($pagesize==100000)
{ ?>
		    <a href="timeperiods_list.php?pagesize=50">geteilt anzeigen</a>
<?} else { ?>
    		<a href="timeperiods_list.php?pagesize=100000">alle anzeigen</a>
<?}
?>
		</td></tr>
	</table>
...
<?
echo "<a target=_blank href='timeperiods_form.php'ONMOUSEOVER=\"window.status='Neuer Eintrag'; return true;\" ONMOUSEOUT=\"window.status='';return true\"><img src='../images/buttons/neuereintrag.gif' border=no></a><br><br>";

HtmlTableFromSQLGlobalsInit(3);

$links[0] = new stdClass();
$colformat[4] = new stdClass();
$links[0]->url = "../admin/timeperiods_form.php?id=";
$links[0]->column = "ID";
$links[0]->param = "";
...
HtmlTableFromSQL ($sql_timeperiods_list, $field, $links, $sortcol, true,$tableparam,$rowpos,$pagesize);
?>
